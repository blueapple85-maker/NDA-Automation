...
// NDA Desk modifications: bounded CFB traversal, strict text-piece validation,
// and textSections() without styled HTML, image carving, or model generation.
/*
...
(function (root, factory) {
  ...
  function parse(input, textOnly) {
    try {
      ...
      if (!bytes || bytes.length < 512 || bytes.length > 10 * 1024 * 1024) return null;
  ...
  docToText.textSections = function (input) { return parse(input, true); };
  ...
  function parseCfb(bytes) {
    ...
    if ((sectorShift !== 9 && sectorShift !== 12) || miniSectorShift !== 6 || dv.getUint16(28, true) !== 0xFFFE) return null;
    if (bytes.length % sectorSize !== 0) return null;
    ...
    if (numMiniFatSectors > sectorCount || numDifatSectors > sectorCount || miniStreamCutoff !== 4096) return null;
    ...
    while (difatSid !== ENDOFCHAIN && difatSid !== FREESECT && difatGuard-- > 0) {
      if (seenDifat.has(difatSid) || seenDifat.size >= numDifatSectors) return null;
      seenDifat.add(difatSid);
      var dbase = sectorOffset(difatSid);
      if (dbase + sectorSize > bytes.length) return null;
    ...
    if (fatSectorLocs.length > sectorCount || new Set(fatSectorLocs).size !== fatSectorLocs.length || fatSectorLocs.some(function (sid) { return sid >= sectorCount; })) return null;
    ...
    function readChain(startSid, sizeLimit) {
      ...
      if (sizeLimit != null && (sizeLimit < 0 || sizeLimit > bytes.length)) throw new Error('Invalid stream size');
      var chunks = [];
      var sid = startSid;
      ...
      while (sid !== ENDOFCHAIN && sid !== FREESECT && guard-- > 0) {
        if (sid >= fat.length || sid >= sectorCount || seen.has(sid)) throw new Error('Invalid or cyclic sector chain');
        seen.add(sid);
      ...
      if (sizeLimit != null && collected < sizeLimit) throw new Error('Truncated stream');
    ...
    var directoryEntries = new Array(numEntries);
    for (var n = 0; n < numEntries; n++) {
      ...
      if (nameLen < 2 || nameLen > 64 || nameLen % 2) return null;
      var name = '';
      if (nameLen > 2) {
      ...
      entry.left = dirDv.getUint32(base + 68, true);
      entry.right = dirDv.getUint32(base + 72, true);
      entry.child = dirDv.getUint32(base + 76, true);
      directoryEntries[n] = entry;
      if ((type === 2 || type === 5) && entry.size > bytes.length) return null;
      if (type === 5) { if (root) return null; root = entry; }
    }
    if (!root) return null;
    // Only root-level streams describe this document; embedded documents do not.
    var dirStack = [root.child], seenDirectory = new Set();
    while (dirStack.length) {
      var dirId = dirStack.pop();
      if (dirId === FREESECT) continue;
      if (seenDirectory.has(dirId) || !directoryEntries[dirId]) return null;
      seenDirectory.add(dirId);
      var directEntry = directoryEntries[dirId];
      if (directEntry.type === 2) {
        if (byName[directEntry.name]) return null;
        byName[directEntry.name] = directEntry;
      }
      dirStack.push(directEntry.left, directEntry.right);
    }
    ...
    function readMiniChain(startSid, sizeLimit) {
      ...
      var seen = new Set();
      while (sid !== ENDOFCHAIN && sid !== FREESECT && guard-- > 0) {
        if (sid >= miniFat.length || seen.has(sid)) throw new Error('Invalid or cyclic mini-sector chain');
        seen.add(sid);
        var off = sid * miniSectorSize;
        if (off >= miniStream.length) throw new Error('Invalid mini-sector offset');
      ...
      if (collected < sizeLimit) throw new Error('Truncated mini-stream');
      return concat(chunks, sizeLimit);
    }
  ...
  function parseWord(wd, cfb, textOnly) {
    ...
    if (fcClx + lcbClx > tableBytes.length) return null;

    var pieces = parsePieceTable(tableBytes, fcClx, lcbClx);
    if (!pieces) return null;
    var totalCharacters = ccpText + ccpFtn + ccpHdd + ccpAtn + ccpEdn + ccpTxbx + ccpHdrTxbx;
    if (ccpText > 500000 || totalCharacters > 1500000 || pieces[0].cpStart !== 0 || pieces[pieces.length - 1].cpEnd < totalCharacters) return null;
    for (var pi = 0; pi < pieces.length; pi++) {
      var pc = pieces[pi];
      if (pc.cpEnd < pc.cpStart || pc.offset + (pc.cpEnd - pc.cpStart) * (pc.compressed ? 1 : 2) > wd.length) return null;
    }
    ...
    if (textOnly && dv.getUint32(fibRgFcLcbStart + 12 * 8 + 4, true) > 4 && chpx === null) return null;
    var isDeleted = makeIsDeleted(chpx);
    if (textOnly) {
      var textDoc = {}, nextCp = 0;
      var stories = [['body', ccpText], ['footnotes', ccpFtn], ['headers', ccpHdd], ['annotations', ccpAtn], ['endnotes', ccpEdn], ['textboxes', ccpTxbx], ['headerTextboxes', ccpHdrTxbx]];
      stories.forEach(function (story) {
        textDoc[story[0]] = extractRange(wd, pieces, nextCp, nextCp + story[1], isDeleted);
        nextCp += story[1];
      });
      textDoc.originalBody = extractRange(wd, pieces, 0, ccpText, null);
      return textDoc;
    }
  ...
  function parsePieceTable(table, fcClx, lcbClx) {
    ...
    while (p < end) {
      ...
      if (clxt === 0x01) {              // Prc: 1 + 2 + cbGrpprl bytes
        ...
        if (cbGrpprl < 0 || p + 3 + cbGrpprl > end) return null;
        p += 3 + cbGrpprl;
    ...
    if (plcOff + plcLen > end || plcLen < 4 || (plcLen - 4) % 12) return null;
  ...
  function collectFkpRuns(wd, pageOff, runs) {
    if (pageOff < 0 || pageOff + 512 > wd.length) throw new Error('Invalid character property page');
    var dv = new DataView(wd.buffer, wd.byteOffset, wd.byteLength);
    var crun = wd[pageOff + 511];
    if (crun > 101) throw new Error('Invalid character property count');
    if (!crun) return;
